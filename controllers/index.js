...
module.exports = {
  ...
  sendNotificacion: catchAsync(async (req,res,next)=>{
    try {
      ...
      let statusRender = status ? 'Confirmado' : 'Rechazado'
      let html = await readFile('./emails/NotificationStatus/notification.html')
      const htmlRenderizado = html.replace('${nameUser}', userName).replace('${name}', name).replace('${status}',statusRender);
      const mailOptions = {
